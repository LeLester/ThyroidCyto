...
def main(args):
    ...
    print("device = ", device, "name = {}".format(name))

    tb_writer = SummaryWriter('runs/{}'.format(name))
    ...
    for epoch in range(args.epochs):
        ...
        weight_path = "weights/{}".format(name)
        if not os.path.exists(weight_path):
            os.makedirs(weight_path)

        torch.save(model.state_dict(), weight_path + "/model-{}.pth".format(epoch))

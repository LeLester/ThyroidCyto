...
def acc(n, l_path, p_path):
    ...
    for file in os.listdir(l_path):
        count = 0
        result = np.load(os.path.join(l_path, file))
        index = np.argsort(result.ravel())[:-n - 1:-1]
        pos = np.unravel_index(index, result.shape)
        label = np.column_stack(pos)
        test_file = np.load(os.path.join(p_path, file))
        index_t = np.argsort(test_file.ravel())[:-n - 1:-1]
        pos_t = np.unravel_index(index_t, test_file.shape)
        test = np.column_stack(pos_t)
        for line in test:
            if line in label:
                count = count + 1
        re.append(count / n)
...
def acc_10(l_path, p_path):
    re = []
    for file in os.listdir(l_path):
        count = 0
        result = np.load(os.path.join(l_path, file))
        n = int(result.shape[0] * result.shape[1] * 0.1)
        if n == 0:
            n = 1
        index = np.argsort(result.ravel())[:-n - 1:-1]
        pos = np.unravel_index(index, result.shape)
        ...
        if (count / n) >= 0.5:
            ...
            print(os.path.join(l_path, file))
    return re


def accelerate_95(f_path):
    rate = []
    for f_dir in os.listdir(f_path):
        temp_dir = os.path.join(f_path, f_dir)
        r = []
        for file in os.listdir(temp_dir):
            t_file = np.load(os.path.join(temp_dir, file))
            r.append(t_file)
        r = np.array(r)
        temp_file = np.argsort(r.flatten())
        temp_file = temp_file.tolist()
        te_file = r.flatten()
        mean_num = 1.0 * np.sum(te_file) / np.count_nonzero(te_file)
        for i in range(te_file.shape[0]):
            if te_file[i] < mean_num:
                te_file[i] = 0.1 * te_file[i]
        p = 0.95 * np.sum(te_file)
        temp = 0
        count = 0
